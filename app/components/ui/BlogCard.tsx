...
const BlogCard = (
  ...
  return (
    ...
    <div ref={blogRef}>
    <Link href={`/blogs/${blog.id}`} className='flex relative flex-col h-[30rem]  gap-3 px-6 pt-4 pb-10 bg-gradient-to-b from-[rgba(250,249,246,0.2)] to-[rgba(250,249,246,0)] rounded-xl border-t-2 border-t-[#FAF9F6]
    ...
    <Button className='bg-[#8FC963] text-white hover:bg-[#8FC963]/70 ' text='اقرأ المزيد' customLink link={`/blogs/${blog.id}`} withMsg msgInput={`اريد حجز موعد لخدمة ${blog.title}`}/>
    </div>
  </div>
  )
}
